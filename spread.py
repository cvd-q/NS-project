...
############################################################
def draw_curve(datadict, N, num_days, confidence=False, stdevdict={}, img_file='curves.png'):
        x = [i+1 for i in range(num_days)]
        fig, ax = plt.subplots()
        ax.plot(x, datadict['susceptible'], 'g',linewidth=0.5)
        ax.plot(x, datadict['infected'], 'r', linewidth=0.5)
        ax.plot(x, datadict['recovered_cum'], 'b', linewidth=0.5)
        ax.plot(x, datadict['dead_cum'], 'k', linewidth=0.5)
        ax.plot(x, datadict['total_inf'], 'y', linewidth=0.5)
        ax.plot(x, datadict['r0'], linewidth=0.5)
        if confidence: #MORE COMPLETE DRAW FUNCTION IS IN My_average_run
                if not stdevdict:
                        print ('stdevdict not specified. Skipping drawing intervals')
                else:
                        ax.fill_between(x, [b-a for a,b in zip(stdevdict['susceptible'], datadict['susceptible'])], [b+a for a,b in zip(stdevdict['susceptible'], datadict['susceptible'])], color='g', alpha=0.1)
                        ax.fill_between(x, [b-a for a,b in zip(stdevdict['infected'], datadict['infected'])], [b+a for a,b in zip(stdevdict['infected'], datadict['infected'])], color='r', alpha=0.1)
                        ax.fill_between(x, [b-a for a,b in zip(stdevdict['recoverd_cum'], datadict['recoverd_cum'])], [b+a for a,b in zip(stdevdict['recoverd_cum'], datadict['recoverd_cum'])], color='b', alpha=0.1)
                        ax.fill_between(x, [b-a for a,b in zip(stdevdict['dead_cum'], datadict['dead_cum'])], [b+a for a,b in zip(stdevdict['dead_cum'], datadict['dead_cum'])], color='k', alpha=0.1)
                        ax.fill_between(x, [b-a for a,b in zip(stdevdict['total_inf'], datadict['total_inf'])], [b+a for a,b in zip(stdevdict['total_inf'], datadict['total_inf'])], color='y', alpha=0.1)
                        # NO R0 VISUALIZATION, MORE COMPLETE DRAW FUNCTION IS IN My_average_run.
        ax.legend(['susceptible', 'infected', 'recovered_cum', 'dead_cum', 'total cases', 'R0'], loc='best', fontsize='x-small')
        plt.xticks(fontsize='x-small')
        plt.yticks(fontsize='x-small')
        plt.savefig(img_file, dpi=500)
        plt.close()        
